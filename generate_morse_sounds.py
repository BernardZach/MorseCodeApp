...
def generate_morse_sounds(trainer):
    ...
    dot_sound.export(os.path.join(static_dir, "dot.wav"), format="wav")
    dash_sound.export(os.path.join(static_dir, "dash.wav"), format="wav")

    print(f"Generated dot.wav ({trainer.dot_duration} ms) and dash.wav ({trainer.dash_duration} ms) in {static_dir}/")
